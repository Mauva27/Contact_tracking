...
class Header:
    ...
    def getSerieInfo(self):
        """
        Method: Generates a dict containing the information for all the files contained in the lif.
        Each Serie gets an index i assigned.
        The dict includes: Filename, N_channels, N_dimensions, 'Channels', X, Y, Z, T lenghts
        """
        st = {}
        for i,s in enumerate(self.getSeriesHeaders()):
            st.setdefault(i,{})
            st[i]['Filename'] = s.getName()
            st[i]['N_channels'] = len(s.getChannels())
            st[i]['N_dimensions'] = len(s.getDimensions())
            st[i].setdefault('Channels',[])
            st[i].setdefault('Dimensions',[])
            for c in s.getChannels():
                st[i]['Channels'].append(channelTag[int(c.getAttribute("ChannelTag"))])
            for d in s.getDimensions():
                if int(d.getAttribute("DimID")) in dimName:
                    st[i]['Dimensions'].append([dimName[int(d.getAttribute("DimID"))], int(d.getAttribute("NumberOfElements"))])
                else:
                    pass
        return st
    ...
    def chooseSerieIndex(self):
        ...
        for i, s in enumerate(self.getSeriesHeaders()):
            ...
            for j,d in enumerate(s.getDimensions()):
                if int(d.getAttribute("DimID")) in dimName:
                    st += " %s%i" % (
    ...
    def chooseSerieHeader(self):
        ...
        Promt for selecting a Serie number.
        """
        print(len(self.getSeriesHeaders()))
        # return self.getSeriesHeaders()[self.chooseSerieIndex()]
        return self.getSerieInfo()
